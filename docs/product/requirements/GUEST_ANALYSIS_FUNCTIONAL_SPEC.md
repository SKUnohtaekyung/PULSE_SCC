...
| SC-012 | 마이페이지 | 알림·설정·로그아웃·페르소나 이미지 보관 공간 | 하단 내비게이션 선택 |

로그인 후 가게 입력·분석 과정의 세부 화면 분리는 미정이다. 인증 후 주요 영역은 홈·분석하기·마이페이지 하단 내비게이션을 따른다.
...
| 요구사항 ID | 기능 | 상세 명세 | 우선순위 |
|---|---|---|---:|
| NAV-001 | 하단 내비게이션 | `홈`·`분석하기`·`마이페이지` 세 항목을 제공한다. 시각적 순서는 미정이다. | P0 |
| NAV-002 | 홈 | 계정에 저장된 분석 결과 1개를 표시한다. | P0 |
| NAV-003 | 분석하기 | 새 가게 정보를 입력하고 분석을 시작하는 흐름으로 진입한다. | P0 |
| NAV-004 | 마이페이지 | 알림·설정·로그아웃·페르소나 이미지 보관 공간을 제공한다. 승인되지 않은 항목을 추가하지 않는다. | P0 |
| NAV-005 | 세부 기능 보류 | 알림·설정의 내용과 페르소나 이미지 보관 개수·삭제 방식은 확정 전까지 구현 요구사항으로 만들지 않는다. | P0 |
...
- **Given** 사용자가 앱의 인증 후 영역에 있고
- **When** 하단 내비게이션을 사용하면
- **Then** 홈·분석하기·마이페이지로 이동할 수 있어야 한다.
- **And** 마이페이지에는 알림·설정·로그아웃·페르소나 이미지 보관 공간을 제공해야 한다.
...
| 하단 내비게이션 세 항목의 시각적 순서 | 내비게이션 구현 전 | NAV-001 |
| 알림·설정 세부 기능과 페르소나 이미지 보관·삭제 정책 | 마이페이지 구현 전 | NAV-004~005 |
