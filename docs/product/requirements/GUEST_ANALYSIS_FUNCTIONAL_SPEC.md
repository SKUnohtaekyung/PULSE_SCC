...
| 문서 상태 | 핵심 결과 IA 반영 / 개발 전 상세 기능명세 |
| 소유 역할 | `role:product` |
...
- 정상 결과에서는 상위 토픽 3개에 대응하는 페르소나를 총 3개 생성한다. 근거가 부족하면 도출된 수만 제공하고 빈 포디움 슬롯으로 한계를 알린다.
...
| FR-011 | 첫 결과 자동 저장과 새 결과 교체·기존 결과 유지 선택 | P0 |
| FR-012 | 홈·분석하기·마이페이지 하단 내비게이션 | P0 |
...
flowchart TD
    ...
    O --> Q[홈에 저장 결과 표시]
    N -- 있음 --> P{사용자 선택}
    P -- 새 결과로 교체 --> R[기존 저장본 교체]
    P -- 기존 결과 유지 --> S[기존 저장본 유지]
    R --> Q
    S --> Q
```
...
| SC-004 | 페르소나별 결과 요약 | 선택한 페르소나의 긍정·부정·인식·우선순위 요약 | 분석 성공 |
| SC-005 | 근거 상세 | 분석 항목에 연결된 익명 근거 리뷰 표시 | 결과 항목 선택 |
| SC-006 | 손님 유형 | TOP3 포디움, 페르소나, 4관점, 근거 리뷰, 한계 고지 | 분석 성공 |
| SC-007 | 페르소나 이미지 | 도출된 페르소나마다 OpenAI API로 생성한 통일된 스타일의 이미지와 생성 사실 고지 | 도출된 모든 페르소나 이미지 생성 성공 |
| SC-008 | 실행 제안 | 리뷰 사실·AI 해석·전문 지식 참고·제안 표시 | 분석 성공 |
| SC-009 | 오류·한계 | 잘못된 주소, 수집 실패, 리뷰 부족, 분석 실패 안내 | 각 실패 발생 |
| SC-010 | 저장 결과 선택 | 첫 결과 자동 저장 또는 새 결과 교체·기존 결과 유지 선택 | 분석 성공 |
| SC-011 | 홈 | 저장된 분석 결과 1개와 포디움 기반 유형별 결과 표시 | 저장 완료 또는 기존 세션 복원 |
| SC-012 | 마이페이지 | 분석 완료·실패 인앱 알림, 알림 설정·서비스 정보, 로그아웃, 현재 저장 결과 이미지 | 하단 내비게이션 선택 |

로그인 후 가게 입력·분석 과정의 세부 화면 분리는 미정이다. 저장 결과가 없는 사용자는 첫 분석을 완료해야 홈·분석하기·마이페이지 하단 내비게이션에 진입할 수 있다.
...
| AUTH-008 | 전화번호 목적 고지 | 전화번호는 자체 계정 필수 정보로 저장하되 MVP에서는 SMS 인증·로그인·복구에 사용하지 않고 중복을 허용한다는 점을 고지한다. | P0 |
| AUTH-009 | 첫 분석 진입 제한 | 새 로그인 또는 세션 복원 후 저장 결과가 없으면 가게 정보 입력으로 이동시키고 첫 분석 완료 전에는 홈·하단 내비게이션 진입을 허용하지 않는다. | P0 |
| AUTH-010 | 토큰 수명과 회전 | Access JWT는 15분, Refresh Token은 30일이며 갱신할 때마다 회전하고 원문을 서버에 저장하지 않는다. | P0 |
| AUTH-011 | 계정 연결 보호 | 동일 이메일의 Google·자체 계정을 자동 연결하지 않고 기존 계정 인증을 요구한다. | P0 |
...
| ANALYSIS-004 | 페르소나별 4개 관점 분석 | 상위 토픽별 페르소나마다 긍정 신호, 부정 신호, 인식, 개선 우선순위를 생성한다. | P0 |
...
| PERSONA-007 | 근거 부족 처리 | 근거를 충족하는 토픽이 3개보다 적으면 도출된 페르소나만 반환하고 나머지는 만들지 않는다. 결과에는 3칸 포디움을 유지하되 부족한 슬롯을 비우고 리뷰 근거가 부족해 3개보다 적게 도출됐음을 표시한다. | P0 |

### 5.5 페르소나 이미지

| 요구사항 ID | 기능 | 상세 명세 | 우선순위 |
|---|---|---|---:|
| IMAGE-001 | OpenAI 이미지 생성 | 도출된 TOP3 페르소나의 비식별 특성을 바탕으로 OpenAI API를 호출해 페르소나별 가상 이미지를 1개씩 생성한다. 정상 결과는 총 3개이며 유형 부족 결과는 실제 도출된 유형 수만큼 생성한다. | P0 |
| IMAGE-002 | 스타일 일관성 | 도출된 모든 이미지는 별도로 확정할 화풍·톤·구성 규칙을 따른다. 규칙 확정 전에는 완료로 판정하지 않는다. | P0 |
| IMAGE-003 | 생성 이미지 고지 | 실제 고객 사진이 아닌 AI 생성 이미지임을 명확하게 표시한다. | P0 |
| IMAGE-004 | 개인 재현 금지 | 실제 리뷰 작성자의 얼굴이나 특정 개인을 재현하도록 요청하지 않는다. | P0 |
| IMAGE-005 | 생성 실패 | 이미지 생성 실패 시 정해진 정책으로 재시도한다. 실제 도출된 모든 페르소나의 이미지가 생성되기 전에는 작업을 완료로 판정하지 않는다. | P0 |
...
| 요구사항 ID | 기능 | 상세 명세 | 우선순위 |
|---|---|---|---:|
| RESULT-001 | 결과 계층 | 분석 메타정보 다음 TOP3 포디움을 표시하고, 선택한 페르소나 아래에 긍정·부정·인식·우선순위 4관점을 표시한다. | P0 |
| RESULT-002 | 근거 탐색 | 각 4관점과 제안에 대표 익명 근거 리뷰 1~2개를 기본 노출하고 나머지는 `근거 리뷰 전체 보기`로 확인할 수 있다. | P0 |
| RESULT-003 | 시각적 구분 | `리뷰에서 확인`과 `AI 해석`을 별도 카드·배지·제목으로 구분한다. 색상만으로 구분하지 않는다. | P0 |
...
| RESULT-010 | 첫 결과 자동 저장 | 저장본이 없는 계정의 첫 분석 결과를 자동 저장하고 홈에 표시한다. | P0 |
| RESULT-011 | 저장본 선택 | 저장본이 있는 상태에서 새 분석이 완료되면 새 결과로 교체하거나 기존 결과를 유지하는 선택을 제공한다. | P0 |
| RESULT-012 | 포디움 최초 선택 | 홈 결과 진입 시 도출된 페르소나가 1개 이상이면 리뷰 수가 가장 많은 1위를 최초 선택한다. | P0 |
| RESULT-013 | 빈 포디움 슬롯 | 유효 토픽이 3개보다 적으면 도출된 유형만 채우고 나머지 포디움 슬롯에 리뷰 근거 부족 이유를 표시한다. 0개면 세 슬롯을 모두 비우고 선택 콘텐츠를 표시하지 않는다. | P0 |
| RESULT-014 | 제안 기본 노출 | 제안에서 리뷰 사실과 검토할 행동을 기본 노출하고 AI 해석과 전문 지식은 펼쳐보기로 제공한다. | P0 |
| RESULT-015 | 오류 배치 | 입력 오류는 필드 가까이, 수집·분석 오류는 진행 화면, 결과 한계는 결과 화면에서 원인·현재 상태·다음 행동과 함께 표시한다. | P0 |

### 5.8 앱 내비게이션과 마이페이지

| 요구사항 ID | 기능 | 상세 명세 | 우선순위 |
|---|---|---|---:|
| NAV-001 | 하단 내비게이션 | 왼쪽부터 `홈`·`분석하기`·`마이페이지` 순서로 제공하고 가운데 `분석하기`를 주요 행동으로 강조한다. | P0 |
| NAV-002 | 홈 | 첫 분석을 완료한 계정에 저장된 분석 결과 1개를 표시한다. 저장 결과가 없으면 가게 정보 입력으로 이동한다. | P0 |
...
8. **결과 제공**
   ...
   - 정상 결과는 이미지 3개, 유형 부족 결과는 실제 도출된 모든 페르소나 이미지가 생성되지 않으면 작업을 완료로 판정하지 않는다.
9. **결과 저장 선택**
   - 계정에 저장본이 없으면 첫 결과를 자동 저장한다.
   - 저장본이 있으면 새 결과로 교체하거나 기존 결과를 유지하는 사용자 선택을 반영한다.
   - 두 선택 모두 계정당 저장 결과 1개 제약을 지킨다.
...
| ANALYZING | 손님 유형과 페르소나별 4개 관점 분석 중 |
...
- 서비스 자체 로그인 비밀번호는 8자 이상, UTF-8 기준 72바이트 이하이며 BCrypt 해시만 저장한다.
- Refresh Token은 해시만 저장하고 회전된 토큰 재사용 시 해당 계정의 활성 세션을 폐기한다.
...
- **When** 포디움에서 페르소나를 선택해 결과를 조회하면
- **Then** 해당 페르소나의 긍정 신호, 부정 신호, 인식, 우선순위가 모두 제공되어야 한다.
...
- **Given** 손님 유형이 생성됐고
- **When** 유형 상세를 조회하면
- **Then** 정상 결과에서는 상위 토픽 3개와 토픽별 페르소나 1개씩 총 3개가 제공되어야 한다.
- **And** 각 페르소나에 하나 이상의 근거 리뷰가 연결되어야 한다.
- **And** 리뷰로 확인되지 않은 연령·성별·직업을 단정형으로 표시하지 않아야 한다.
- **Given** 근거를 충족한 토픽이 3개보다 적고
- **When** 포디움을 표시하면
- **Then** 도출된 페르소나만 채우고 나머지 슬롯은 비워야 한다.
- **And** 분석에 활용할 리뷰 근거가 부족해 3개보다 적게 도출됐다는 메시지를 표시해야 한다.

### AC-07 사실과 AI 해석 구분

- **Given** 분석 결과와 제안이 생성됐고
- **When** 사용자가 결과를 읽으면
- **Then** `리뷰에서 확인`과 `AI 해석`을 별도 카드·배지·제목으로 구분할 수 있어야 한다.
- **And** 모든 제안에는 리뷰 사실과 검토할 행동이 기본으로 함께 보여야 한다.
- **And** AI 해석과 전문 지식은 펼쳐서 확인할 수 있어야 한다.

### AC-08 페르소나 이미지 완료

- **Given** 근거를 충족한 페르소나가 생성됐고
- **When** 이미지 생성 단계가 완료되면
- **Then** 도출된 각 페르소나에 대응하는 OpenAI 생성 이미지가 1개씩 있어야 한다. 정상 결과에서는 총 3개여야 한다.
...
### AC-12 계정당 저장 1개와 저장본 선택

- **Given** 계정에 저장된 분석 결과가 없고
- **When** 첫 분석이 완료되면
- **Then** 결과가 자동 저장되고 홈에 표시되어야 한다.
- **Given** 계정에 저장 결과가 이미 1개 있고
- **When** 새 분석이 완료되면
- **Then** 새 결과로 교체하거나 기존 결과를 유지하는 선택을 제공해야 한다.
- **And** 교체를 선택하면 홈에 새 결과를, 유지를 선택하면 홈에 기존 결과를 표시해야 한다.
- **And** 어떤 선택에서도 계정의 저장 결과는 1개여야 한다.

### AC-13 하단 내비게이션과 마이페이지

- **Given** 사용자가 로그인했지만 저장된 분석 결과가 없고
- **When** 앱에 진입하면
- **Then** 가게 정보 입력으로 이동하고 첫 분석 완료 전에는 홈과 하단 내비게이션에 진입할 수 없어야 한다.
- **Given** 사용자가 첫 분석을 완료했고
- **When** 하단 내비게이션을 사용하면
...
- **Given** 저장된 분석 결과가 있고
- **When** 사용자가 홈 결과에 진입하면
- **Then** 분석 플랫폼·리뷰 수·수집 시점·완료 시점이 TOP3 포디움보다 먼저 표시되어야 한다.
- **And** 도출된 페르소나가 1개 이상이면 포디움의 1위가 최초 선택되고 해당 페르소나의 4관점·근거·제안이 포디움 아래에 표시되어야 한다.
- **And** 도출된 페르소나가 0개이면 세 포디움 슬롯을 모두 비우고 근거 부족 안내만 표시해야 한다.

### AC-15 오류와 한계 배치

- **Given** 입력 검증, 수집·분석 또는 결과 한계 상태가 발생했고
- **When** 사용자에게 해당 상태를 표시하면
- **Then** 입력 오류는 해당 필드 가까이, 수집·분석 오류는 진행 화면, 결과 한계는 결과 화면에 표시되어야 한다.
- **And** 각 상태에는 원인·현재 상태·사용자가 할 수 있는 다음 행동이 함께 제공되어야 한다.
...
| 계정 복구·탈퇴 정책 | 인증 기능 확장 전 | AUTH-002~008 |
...
| 기존 결과 유지 시 미저장 새 결과의 접근 범위·최대 보관 시간 | 작업 큐·삭제 배치 구현 전 | RESULT-009~011 |
...
| FR-002 | ANALYSIS-004~009, RESULT-001~003 |
| FR-003 | PERSONA-001~007, RESULT-013 |
| FR-004 | IMAGE-001~006 |
| FR-005 | ADVICE-001~008, RESULT-014 |
| FR-006 | ANALYSIS-006, ADVICE-002~003, RESULT-003 |
| FR-007 | RESULT-001~007, RESULT-012~015 |
| FR-008 | AUTH-001~009 |
| FR-009 | REVIEW-007~008, REVIEW-010~011 |
| FR-010 | INPUT-001~007 |
| FR-011 | RESULT-008~011 |
| FR-012 | AUTH-009, NAV-001~007 |
