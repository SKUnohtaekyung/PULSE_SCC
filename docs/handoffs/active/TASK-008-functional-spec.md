...
백엔드 실행 스택 관련 항목은 후속 [TASK-009](TASK-009-backend-bootstrap.md)와 [ADR-006](../../decisions/ADR-006-backend-bootstrap.md)에서 해소했다. 남은 항목은 다음과 같다.

1. Expo SDK·React Native 버전과 development build 사용 여부
2. 전화번호의 용도(SMS 인증, 계정 복구, 연락처 저장 여부)와 가입·복구·탈퇴 정책
3. 인증 토큰 수명·회전·폐기 정책
4. OpenAI 이미지 모델·크기·비용·재시도 정책
5. 최소 Android OS와 지원 기기
6. 리뷰 작성일을 알 수 없을 때 2년 경고 처리
7. 원본 리뷰·분석 결과·인증 데이터 보관 및 삭제 기간
8. 허용 네이버 URL 도메인 목록
9. 주소·대표 메뉴·최근 이전·리뉴얼·업주 변경 시점의 선택 입력 여부
10. 비밀번호 규칙과 전화번호 중복 가입 정책
11. `기타` 업종 세부값 입력 여부
12. 이용약관·개인정보 처리 동의와 이력 보관 방식
13. 기존 결과 유지 시 미저장 새 결과의 세션 내 접근 범위·최대 보관 시간
14. 동시 분석 작업 수
15. 유형별 최소 리뷰 수를 별도로 도입할지
16. 가게 지정·분석 대기 화면의 세부 시각 구성
17. 포디움을 스크롤 중 상단에 붙이는 sticky 동작 여부
18. 실제 OpenAPI 작성 도구와 schema/types 생성 방향
19. Spring Boot–Python 내부 HTTP의 인증·timeout·재시도 값
20. PostgreSQL schema/role 분리와 보관·삭제 배치 방식
21. 저장 결과 없음과 진행 중 결과 조회 등 API 오류별 최종 HTTP 상태
