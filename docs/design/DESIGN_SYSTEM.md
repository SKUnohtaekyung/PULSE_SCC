...
Google 소셜 로그인과 서비스 자체 로그인은 MVP 범위다. 대시보드, 리뷰 답글 관리, 홍보영상, 인플루언서, 요금제 화면은 적용 범위가 아니다. 해당 기능이 PRD에 추가되기 전에는 이 문서에 화면 규칙을 만들지 않는다.
...
TOP3 페르소나 이미지 3개는 P0 완료 조건이므로 하나라도 없으면 정상 완료 상태로 표현하지 않는다.
...
| BottomNavigation | 홈·분석하기·마이페이지 이동. 항목의 시각적 순서는 TBD | TBD |
| PersonaImageStorage | 마이페이지의 페르소나 이미지 보관 공간. 세부 정책은 TBD | TBD |
...
- 가게 이름, 업종, 네이버 가게 URL을 각각 구분된 필수 입력으로 제공한다.
- 업종은 한식·중식·일식·양식·카페/디저트·주점·기타 중 하나를 선택하게 한다.
...
| SC-007 페르소나 이미지 | AI 생성 고지와 대체 텍스트를 제공하고, 3개 중 하나라도 실패하면 완료 상태로 표시하지 않음 |
...
| SC-011 홈 | 저장된 분석 결과 1개와 포디움 기반 유형별 결과를 표시 |
| SC-012 마이페이지 | 알림·설정·로그아웃·페르소나 이미지 보관 공간만 제공하고 미확정 세부 기능을 만들지 않음 |
...
- 기존 로그인·회원가입의 2분할 3D 레이아웃. 로그인 기능은 포함하지만 이 레이아웃은 자동 이식하지 않는다
...
| 실패 | 리뷰 부족, 수집 오류, 모델 오류, 이미지 생성 실패와 완료 차단 상태가 구분되는가 |
